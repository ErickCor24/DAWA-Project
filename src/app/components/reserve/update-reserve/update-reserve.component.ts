...
export class UpdateReserveComponent implements OnInit {
  ...
  fechaMin: Date = new Date();
    private selectedVehiclePricePerDay = 0;
  ...
  constructor(
   ...
   ngOnInit(): void {
    ...
    // 3) Cargar sólo los vehículos disponibles
    this.vehicleService.getVehicles().subscribe(vList => {
      this.vehicles = vList;

      // 4) Cargar datos de la reserva a editar
      ...
      this.reserveService.getReserve(id).subscribe(r => {
        ...
        // 5) Traer el vehículo reservado aunque esté no disponible
        this.vehicleService.getVehicle(r.idVehicle).subscribe(veh => {
          this.selectedVehicleName = `${veh.brand} ${veh.model}`;

          // 6) Asegurar que el reserved vehicle esté en el array para el cálculo
        ...
        // 8) Suscribirse a cambios para recalcular
        this.reserveForm.get('pickupDate')!.valueChanges
          .subscribe(() => this.calculatePrice());
        this.reserveForm.get('dropoffDate')!.valueChanges
          .subscribe(() => this.calculatePrice());
        // idVehicle no cambia en Update, por eso no lo suscribimos
      });
    });

    // 9) Cargar agencias (temporal)
